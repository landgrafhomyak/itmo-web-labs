...
class AreaCheckServlet : HttpServlet() {
    override fun doGet(req: HttpServletRequest, resp: HttpServletResponse) {
        ...
        val x = xRaw?.let(AreaCheckerImpl::validateX)
        val y = yRaw?.let(AreaCheckerImpl::validateY)
        val r = rRaw?.let(AreaCheckerImpl::validateR)
        val isDataValid: Boolean
        val result: Boolean
        if (x != null && y != null && r != null) {
            isDataValid = true
            result = AreaCheckerImpl.check(x, y, r)
        } else {
            isDataValid = false
            result = false
        }
